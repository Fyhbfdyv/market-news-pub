 ...
 *
 * Both pages (Vocab Trainer + News reader) import this module. It owns one
 * slice of localStorage and exports the shared `speaker` (see speech-core.js),
 * which resolves its voice through `voiceFor()` on every speak. Because all
 * narration funnels through the same store, changing the voice in the settings
 * panel applies *everywhere* — no per-page wiring, no duplication (DRY).
 *
 * It is self-contained: on import it injects its own "⚙" button into the page
 * header and a settings modal into <body>.
 *
 * Two Web Speech API facts drive the design:
 *   1. speechSynthesis.getVoices() loads ASYNCHRONOUSLY — the first call often
 *      returns [] (always on Safari 27+), and the browser fires a
 *      "voiceschanged" event when ready.
...
import { createSpeaker } from "./speech-core.js";
...
const BUCKETS = [
  { key: "en", lang: "en-US", label: "English voice", sample: "This is a test of the English voice." },
  { key: "zh", lang: "zh-TW", label: "Chinese voice", sample: "這是中文語音的測試。" },
];
...
/**
 * Resolve the live SpeechSynthesisVoice for a BCP-47 language ("en-US").
 *
 * Order of preference: the user's saved choice → the browser's default voice
 * for that language → null (let the browser decide from utterance.lang).
 */
...
/** The shared speaker every page uses; a silent no-op without Web Speech. */
export const speaker = speechSupported
  ? createSpeaker({ synth: speechSynthesis, Utterance: SpeechSynthesisUtterance, voiceFor })
  : { speak: async () => false, stop() {} };
...
function buildModal() {
  ...
  for (const bucket of BUCKETS) {
    ...
    test.onclick = () => speaker.speak(bucket.sample, { lang: bucket.lang });
...
// ---- Boot (modules run after the document is parsed) -----------------------
...
if (speechSupported) {
  ...
  speechSynthesis.addEventListener("voiceschanged", () => populateSelects(selects));
}
